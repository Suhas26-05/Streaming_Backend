...
def find_profile(db: Session, user_id: str, profile_name: str): 
    return db.query(Profile).filter(Profile.userId == user_id, Profile.profile_name == profile_name).first()
...
def get_profile(db: Session, user_id: str, profile_id: int): 
    return db.query(Profile).filter(Profile.id == profile_id, Profile.userId == user_id).first()
...
def create_user_session(db: Session, user_id: str, profile_id: int):
    session = UserSession(userId=user_id, profile_id=profile_id, login_time=datetime.now(), is_active=1)
    db.add(session)
    db.commit()
    db.refresh(session)
